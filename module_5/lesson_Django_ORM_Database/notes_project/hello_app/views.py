...
def notebook_list(request):
    ...
    Кожен запис має annotate(note_count) — з selectors.get_user_notebooks().
    """
...
def notebook_edit(request, pk):
    ...
    get_object_or_404 з user=request.user → захист від доступу до чужих записників.
    """
...
def notebook_delete(request, pk):
    ...
    on_delete=SET_NULL → нотатки НЕ видаляються, тільки від'єднуються!
    """
    notebook = get_object_or_404(Notebook, pk=pk, user=request.user)
